...
/**
 * ============================================================================
 * MIGAKU TO ANKI DECK EXPORTER & LEARNING ASSISTANT
 * ============================================================================
 * A userscript that extracts local Migaku SRS decks/history and packages them
 * into standard Anki .apkg files. In addition, it integrates an AI study
 * assistant (MigakuGPT) powered by Google's Gemini API.
 */

/**
 * CONFIG
 * Central configuration store for constants, settings keys, and fields.
 */
const CONFIG = {
    ...
    CHAT_API_KEY_STORAGE: "migaku_gemini_api_key",
...
/**
 * Utils
 * Miscellaneous UI, logging, and DOM safety helper functions.
 */
const Utils = {
...
/**
 * Storage
 * Wraps localStorage access for configuration settings, deck mappings, and Gemini API keys.
 */
const Storage = {
    saveSettings: (obj) => {
        try {
...
/**
 * Progress
 * Manages the generation, updates, and visibility of the exporter's visual progress bar.
 */
const Progress = {
    ensureUI: () => {
...
/**
 * MediaProcessor
 * Handles file decompression (Gzip decoding for Migaku's internal DB) and canvas-based image resizing.
 */
...
/**
 * FirebaseAuth
 * Extracts existing Firebase authorization states from local IndexedDB databases to query Migaku remote media servers.
 */
const FirebaseAuth = {
...
/**
 * DatabaseOps
 * Houses all raw database queries that interface with the deserialized SQL database of the Migaku study site.
 */
const DatabaseOps = {
...
/**
 * FieldMapper
 * Builds and converts deck text contents into configured note output columns.
 */
const FieldMapper = {
    getFieldNames: () => {
...
/**
 * DeckProtection
 * Validates that cards do not belong to restricted course content to protect Migaku academy materials.
 */
...
/**
 * AnkiBuilder
 * Injects initial collection schema tables, metadata models, templates, and review limits.
 */
const AnkiBuilder = {
...
/**
 * MigakuGPT
 * Handles floating visual assistant widgets, drag operations, system prompt creation, and Gemini queries.
 */
const MigakuGPT = {
    isOpen: false,
    chatHistory: [],
    ...
    createChatWindow: () => {
      ...
      <div class="mgk-chat-messages" id="mgkChatMessages">
        <div id="mgkApiKeySetup" class="mgk-api-key-setup">
          <p>Enter your Google Gemini API key to use MigakuGPT:</p>
          <input type="password" class="mgk-api-key-input" id="mgkApiKeyInput" placeholder="AIzaSy...">
          <br>
          ...
          <p style="font-size: 0.75rem; margin-top: 12px; opacity: 0.7;">
            Stored locally only. Get one at <a href="https://aistudio.google.com/" target="_blank" style="color: #4f46e5;">Google AI Studio</a>
          </p>
        </div>
      </div>
    ...
    sendMessage: async () => {
        ...
        try {
            const response = await MigakuGPT.callGemini(message);
    ...
    callGemini: async (message) => {
        ...
        if (functionCallPart) {
            ...
            if (name === "web_search") {
                const searchResults = await MigakuGPT.performWebSearch(args.query, args.max_results || 3);

                // Add the model's functionCall turn to contents history
...
/**
 * MappingModal
 * Visual modal interface for mapping Migaku database fields to custom Anki fields.
 */
const MappingModal = {
    create: () => {
...
/**
 * MediaHandler
 * Identifies, fetches, and caches media assets from the Migaku remote server.
 */
const MediaHandler = {
...
/**
 * ExportProcessor
 * Coordinates SQLite card translations, scheduling offsets, ZIP building, and file downloads.
 */
...
/**
 * UI
 * Injects styling rules, export config forms, native menu overlays, and route listeners.
 */
const UI = {
...
/**
 * TutorialManager
 * Displays spotlight overlays and tutorial prompts to guide first-time users.
 */
